...
def detect_try(request, project_id):
    if request.method == 'POST':
        ...
        image_video_path = FileInfo.objects.filter(updated_by=request.tracer.user.id, name=file_object.name).first().file_path
        # print(image_path)
        if image_video_path and threshold:
            threshold = float(threshold)

            file_name, file_extension = os.path.splitext(file_object.name)
            new_image_video_name, new_image_video_path = detect_get_unique_file_name(user_directory, file_name, file_extension)
            new_image_video_path = new_image_video_path.replace('\\', '/')

            detect_try_(image_video_path, threshold, new_image_video_path)
            ...
            file_info = FileInfo(
                ...
                file_type=2,
                file_path=new_image_video_path
            )
            file_info.save()  # 保存到数据库
            return JsonResponse({'status': 'success', 'data': new_image_video_path})
        else:
